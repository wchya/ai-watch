...
export function SLOView({ navigate }: { navigate: (view: 'reliability' | 'incidents' | 'maintenance') => void }) {
  ...
  const load = useCallback(async (quiet = false) => { const version = ++loadVersion.current; if (!quiet) setLoading(true); try { const next = await api.slos(); if (version !== loadVersion.current) return; setItems(next); setError('') } catch (cause) { if (version === loadVersion.current) setError(cause instanceof Error ? cause.message : 'SLO 读取失败') } finally { if (version === loadVersion.current) setLoading(false) } }, [])
  ...
  const run = async (key: string, operation: () => Promise<ServiceLevelObjective>, success: string) => { if (busy) return false; setBusy(key); setError(''); try { const next = await operation(); setItems(current => current.map(item => item.groupId === next.groupId ? next : item)); setMessage(success); await refreshAfter(); return true } catch (cause) { setError(cause instanceof Error ? cause.message : 'SLO 操作失败'); return false } finally { setBusy('') } }
  ...
  return <div className="page slo-page"><section className="page-heading slo-heading"><div><span className="eyebrow"><Target/>Service level objective</span><h1>看清可靠性还能消耗多久。</h1><p>用错误预算衡量 Provider Group 的可用性目标，快速识别正在加速恶化的线路。</p></div><button className="secondary" disabled={loading} onClick={() => void load()}><RefreshCw className={loading ? 'spinning' : ''}/>刷新</button></section>
    ...
    <div className="slo-filters" role="group" aria-label="SLO 状态筛选">{([['all','全部'],['exhausted','已耗尽'],['critical','即将耗尽'],['burning','快速消耗'],['healthy','健康'],['insufficient','样本不足'],['disabled','已暂停']] as const).map(([value,label]) => <button className={filter === value ? 'active' : ''} aria-pressed={filter === value} onClick={() => setFilter(value)} key={value}>{label}</button>)}</div>
    {error && <div className="error-banner" role="alert"><AlertCircle/><div><strong>SLO 操作失败</strong><span>{error}</span></div><button onClick={() => void load()}>重试</button></div>}{message && <div className="toast-inline success" role="status"><CheckCircle2/>{message}</div>}
...
function SLOCard({ item, busy, edit, run, navigate }: { item: ServiceLevelObjective; busy: string; edit: () => void; run: (key:string, operation:()=>Promise<ServiceLevelObjective>, success:string)=>Promise<boolean>; navigate:(view:'reliability'|'incidents'|'maintenance')=>void }) {
