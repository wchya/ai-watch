...
export function JobDetail({ initial, close, onChanged }: { initial: JobSummary; close: () => void; onChanged: () => void }) {
...
export function SettingsView({ onThemeChanged, onBrowserNotificationsChanged }: { onThemeChanged: (theme: AppSettings['uiTheme']) => void; onBrowserNotificationsChanged: (enabled: boolean) => void }) {
  ...
  const save = async () => { if (!settings) return; setSaving(true); setMessage(''); try { const saved = await api.saveSettings(settings); setSettings(saved); onThemeChanged(saved.uiTheme); onBrowserNotificationsChanged(saved.browserNotifications); setMessageTone('success'); setMessage('设置已保存') } catch (e) { setMessageTone('error'); setMessage(e instanceof Error ? e.message : '保存失败') } finally { setSaving(false) } }
  ...
  const toggleBrowserNotifications = async () => {
    ...
    if (!settings.browserNotifications) {
      const permission = browserPermission === 'default' ? await Notification.requestPermission() : browserPermission
      enabled = permission === 'granted'
    }
    patch('browserNotifications', enabled)
    api.setBrowserNotifications(enabled)
    onBrowserNotificationsChanged(enabled)
  }
  return <div className="page settings-page">
    ...
    {loading ? <div className="settings-loading"><LoaderCircle className="spinning"/>正在读取设置</div> : settings && <div className="settings-grid">
      ...
      <section className="panel settings-panel notification-panel"><PanelTitle title="通知渠道" detail="浏览器权限留在本机；钉钉凭证在服务端加密"/><div className="notification-list"><div className="notification-card"><div className="notification-icon browser"><Bell/></div><div><strong>浏览器通知</strong><span>{browserPermission === 'granted' ? '权限已允许 · 开关即时生效' : browserPermission === 'denied' ? '权限已被浏览器阻止' : browserPermission === 'unsupported' ? '当前浏览器不支持系统通知' : '替代容器中不可用的 macOS 通知'}</span></div><button className={`switch ${settings.browserNotifications ? 'on' : ''}`} aria-label="切换浏览器通知" aria-pressed={settings.browserNotifications} disabled={browserPermission === 'unsupported' || browserPermission === 'denied'} onClick={() => void toggleBrowserNotifications()}><i/></button></div><DingTalkConfigCard onConfigured={configured => setSettings(current => current ? { ...current, dingTalkConfigured: configured } : current)}/></div><div className="secret-note"><KeyRound/><span>Webhook 明文不会返回浏览器；保存成功后输入框会立即清空。</span></div></section>
