...
export function MaintenanceView() {
  ...
  const load = useCallback(async (quiet = false) => { const version = ++loadVersion.current; if (!quiet) setLoading(true); try { const next = await api.maintenanceWindows(); if (version !== loadVersion.current) return; setItems(next); setError('') } catch (cause) { if (version === loadVersion.current) setError(cause instanceof Error ? cause.message : '维护窗口读取失败') } finally { if (version === loadVersion.current) setLoading(false) } }, [])
  ...
  const merge = useCallback((next: MaintenanceWindow) => setItems(current => current.map(item => item.groupId === next.groupId ? next : item)), [])
  const run = async (key: string, operation: () => Promise<MaintenanceWindow>, success: string) => { if (busy) return false; setBusy(key); setError(''); setMessage(''); try { const next = await operation(); merge(next); setMessage(success); await refreshAfter(); return true } catch (cause) { setError(cause instanceof Error ? cause.message : '维护窗口操作失败'); return false } finally { setBusy('') } }
  const start = (item: MaintenanceWindow, seconds: number) => { const minutes = seconds / 60; void confirmAction({ title: '开启维护窗口', message: `立即为“${item.groupName}”开启 ${minutes} 分钟维护窗口？`, detail: '窗口期间继续记录请求和事故，但暂停新通知、备用验证与自动切换。', confirmLabel: `开启 ${minutes} 分钟`, action: async () => { const until = new Date(Date.now() + seconds * 1000).toISOString(); const next = await api.startMaintenance(item.groupId, { until }); merge(next); setMessage(`${item.groupName} 维护窗口已开始`); await refreshAfter() } }) }
  const extend = (item: MaintenanceWindow, seconds: number) => { const minutes = seconds / 60; void confirmAction({ title: '延长维护窗口', message: `将“${item.groupName}”的维护窗口延长 ${minutes} 分钟？`, detail: `当前窗口结束时间将顺延 ${minutes} 分钟。`, confirmLabel: `延长 ${minutes} 分钟`, tone: 'warning', action: async () => { const next = await api.extendMaintenance(item.groupId, seconds); merge(next); setMessage(`${item.groupName} 维护窗口已延长`); await refreshAfter() } }) }
  const end = (item: MaintenanceWindow) => { void confirmAction({ title: '提前结束维护', message: `立即结束“${item.groupName}”的维护窗口？`, detail: '结束后通知、备用验证和自动切换将恢复正常。', confirmLabel: '结束维护', tone: 'danger', action: async () => { const next = await api.endMaintenance(item.groupId); merge(next); setMessage(`${item.groupName} 维护窗口已结束`); await refreshAfter() } }) }
  ...
  return <div className="page maintenance-page"><section className="page-heading maintenance-heading"><div><span className="eyebrow"><CalendarClock/>Maintenance control</span><h1>维护期间继续记录，但暂时保持安静。</h1><p>集中管理 ProviderGroup 维护窗口；只抑制新通知、备用验证与自动切换，不停止已有任务。</p></div><button className="secondary" disabled={loading} onClick={() => void load()}><RefreshCw className={loading ? 'spinning' : ''}/>刷新</button></section>
    ...
    <div className="maintenance-filters" role="group" aria-label="维护窗口状态筛选">{([['all', '全部'], ['active', '进行中'], ['scheduled', '即将开始'], ['ended', '已结束'], ['none', '未设置']] as const).map(([value, label]) => <button className={filter === value ? 'active' : ''} aria-pressed={filter === value} key={value} onClick={() => setFilter(value)}>{label}</button>)}</div>
    {error && <div className="error-banner" role="alert"><AlertCircle/><div><strong>维护窗口操作失败</strong><span>{error}</span></div><button onClick={() => void load()}>重试</button></div>}{message && <div className="toast-inline success" role="status"><CheckCircle2/>{message}</div>}
