...
export function ComparisonHistoryView() {
  ...
  const positionedSelection = useRef('')

  const load = useCallback(async (quiet = false) => {
    const version = ++listVersion.current
    if (!quiet) setLoading(true)
    try { const result = await api.scenarioComparisons(filter || undefined); if (version !== listVersion.current) return; setItems(result.items); setRetentionLimited(result.retentionLimited); setError(''); if (!selectedId && result.items[0]) setSelectedId(result.items[0].id) }
    catch (cause) { if (version === listVersion.current) setError(cause instanceof Error ? cause.message : '对比历史读取失败') }
    finally { if (version === listVersion.current) setLoading(false) }
  }, [filter, selectedId])

  const loadDetail = useCallback(async (id: string, quiet = false) => {
    const version = ++detailVersion.current
    if (!id) { setSelected(null); setDetailLoading(false); return }
    if (!quiet) setDetailLoading(true)
    try { const value = await api.scenarioComparison(id); if (version !== detailVersion.current) return; setSelected(value); setDetailError(''); setItems(current => current.map(item => item.id === value.id ? value : item)) }
    catch (cause) { if (version === detailVersion.current) { setSelected(null); setDetailError(cause instanceof Error ? `${cause.message}；该批次可能已超过事件保留期限。` : '对比历史已过期') } }
  ...
  useEffect(() => {
    if (!selectedId || positionedSelection.current === selectedId) return
    const index = items.findIndex(item => item.id === selectedId)
    if (index < 0) return
    positionedSelection.current = selectedId
  ...
  useEffect(() => {
    ...
    const refresh = () => { if (!document.hidden) { void load(true); if (selectedId) void loadDetail(selectedId, true) } }
  ...
  const pageItems = useMemo(() => items.slice((page - 1) * LIST_PAGE_SIZE, page * LIST_PAGE_SIZE), [items, page])
  ...
  return <div className="page comparisons-page"><section className="page-heading comparisons-heading"><div><span className="eyebrow"><History/>Comparison history</span><h1>每一次线路对比，都能重新打开。</h1><p>从有界结构化事件恢复场景、Provider 排名、错误类型和 Request ID；历史过期时会明确提示。</p></div><button className="secondary" disabled={loading} onClick={() => void load()}><RefreshCw className={loading ? 'spinning' : ''}/>刷新历史</button></section>
    ...
    <div className="comparison-filters" role="group" aria-label="对比状态筛选">{([['', '全部'], ['running', '运行中'], ['completed', '已完成'], ['partial_failed', '含失败']] as const).map(([value, label]) => <button key={value} className={filter === value ? 'active' : ''} aria-pressed={filter === value} onClick={() => setFilter(value)}>{label}</button>)}</div>
    {retentionLimited && <div className="comparison-retention"><AlertCircle/>列表只展示事件保留范围内最近 500 个批次。</div>}{error && <div className="error-banner" role="alert"><AlertCircle/><div><strong>对比历史读取失败</strong><span>{error}</span></div><button onClick={() => void load()}>重试</button></div>}
    <section className="comparisons-layout"><div className="panel comparison-history-list">{loading && !items.length ? <div className="comparison-loading"><LoaderCircle className="spinning"/>正在读取对比历史</div> : items.length ? <>{pageItems.map(item => <button key={item.id} className={selectedId === item.id ? 'active' : ''} onClick={() => choose(item.id)}><span className={`comparison-batch-status ${item.status}`}>{statusLabel(item.status)}</span><div><strong>{item.scenarioName}</strong><small>{item.cli === 'codex' ? 'Codex' : 'Claude'} · {item.items.length} 个 Provider</small><em>{dateTime(item.createdAt)}</em></div><b>{item.items.filter(row => row.status === 'success').length}/{item.items.length}</b></button>)}<ListPagination page={page} total={items.length} label="对比历史分页" onPageChange={setPage}/></> : <div className="comparison-empty"><BarChart3/><strong>还没有对比历史</strong><span>从测试场景运行一次多线路对比后会显示在这里。</span></div>}</div>
      <div className="panel comparison-history-detail" aria-busy={detailLoading}>{detailLoading ? <div className="comparison-loading"><LoaderCircle className="spinning"/>正在恢复批次详情</div> : selected ? <><header><div><span>{selected.id}</span><h2>{selected.scenarioName}</h2><small>{selected.cli === 'codex' ? 'Codex' : 'Claude'} · {dateTime(selected.createdAt)}</small></div><span className={`comparison-batch-status ${selected.status}`}>{statusLabel(selected.status)}</span></header><div className="comparison-detail-actions"><button className="primary" disabled={rerunning || selected.status === 'running'} onClick={() => void rerun()}>{rerunning ? <LoaderCircle className="spinning"/> : <RotateCcw/>}{rerunning ? '启动中' : '按原集合重跑'}</button></div>{detailError && <div className="form-error" role="alert"><AlertCircle/>{detailError}</div>}<div className="comparison-ranking">{ranked.map((row, index) => <article className={statusTone(row.status)} key={`${row.providerId}:${row.jobId}`}><b>{index + 1}</b><span className="comparison-result-icon">{row.status === 'success' ? <CheckCircle2/> : statusTone(row.status) === 'running' ? <LoaderCircle className="spinning"/> : <XCircle/>}</span><div><strong>{row.providerName || row.providerId || '当前配置'}</strong><small>{row.errorType || row.error || row.responseExcerpt || row.jobId || '等待请求结果'}</small></div><span><Clock3/>{row.durationMillis != null ? `${(row.durationMillis / 1000).toFixed(2)}s` : '—'}</span>{row.requestId ? <button onClick={() => openRequest(row.requestId!)}><ExternalLink/>请求详情</button> : <em>{row.status}</em>}</article>)}</div></> : <div className="comparison-empty"><AlertCircle/><strong>{detailError ? '批次历史不可用' : '选择一个对比批次'}</strong><span>{detailError || '查看 Provider 排名和关联请求。'}</span></div>}</div></section>
  </div>
}
