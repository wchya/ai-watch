...
function JobRow({ job, open }: { job: JobSummary; open: () => void }) {
  ...
  return <button className="job-row" onClick={open}><CliIcon cli={job.cli}/><div className="job-main"><div><strong>{cliLabel(job.cli)} · {modeLabel(job.mode)}</strong><StatusPill status={job.status}/></div><span>{job.providerName || job.providerId || '当前配置'}{job.model ? ` · ${job.model}` : ''}</span></div><div className="job-stat"><span>尝试次数</span><strong>{job.attemptCount}</strong></div><div className="job-stat"><span>{job.mode === 'keepalive' ? '下次执行' : '已运行'}</span><strong>{seconds ? `${seconds}s` : formatAgo(job.startedAt)}</strong></div><ChevronRight className="row-arrow"/></button>
}
