...
[![AI Watch 极昼控制台总览](docs/images/dashboard.png)](docs/images/dashboard.png)

截图由当前前端通过 Playwright 和内置脱敏演示数据生成，不需要真实 Provider 凭证。

## 工作流

[![AI Watch 从 Provider 配置到异常处置的工作流](docs/diagrams/system-workflow.png)](docs/diagrams/system-workflow.png)

系统先验证线路，再让计划持续采集请求事实；异常进入事件、请求详情和事故时间线后，由操作者选择复测、维护窗口或备用线路。Provider Group 自动切换只影响显式绑定的 AI Watch 计划。

[可编辑的工作流 draw.io 源文件](docs/diagrams/system-workflow.drawio)
...
<table>
  <tr>
    <td width="50%"><strong>Provider</strong><br/>发现只读配置、维护加密手填 Provider，并为每条线路选择代理策略。<br/><a href="docs/images/providers.png"><img src="docs/images/providers.png" alt="Provider 配置页面"/></a></td>
    <td width="50%"><strong>验证中心</strong><br/>保存可复用场景，比较多条 Provider 线路，并重新打开历史请求事实。<br/><a href="docs/images/validation.png"><img src="docs/images/validation.png" alt="验证中心对比历史页面"/></a></td>
  </tr>
  <tr>
    <td width="50%"><strong>自动化</strong><br/>管理计划、Provider Group、建议式或自动故障切换，以及集中维护窗口。<br/><a href="docs/images/automation.png"><img src="docs/images/automation.png" alt="自动化故障切换页面"/></a></td>
    <td width="50%"><strong>稳定性</strong><br/>查看成功率、延迟、异常窗口、SLO、错误预算和事故处置建议。<br/><a href="docs/images/stability.png"><img src="docs/images/stability.png" alt="稳定性可靠性分析页面"/></a></td>
  </tr>
  <tr>
    <td width="50%"><strong>设置与通知</strong><br/>配置任务、通知路由与 Mihomo 订阅，敏感值保存后不再明文返回。<br/><a href="docs/images/settings.png"><img src="docs/images/settings.png" alt="设置中的系统诊断页面"/></a></td>
    <td width="50%"><strong>总览</strong><br/>聚合当前 Provider、待处置异常、运行任务、计划和环境健康状态。<br/><a href="docs/images/dashboard.png"><img src="docs/images/dashboard.png" alt="AI Watch 总览页面"/></a></td>
  </tr>
</table>
...
| `AI_WATCH_MIHOMO_CONTROLLER_URL` | `http://mihomo:9090` | 用于订阅热重载的私有 Controller |
| `AI_WATCH_MIHOMO_TEST_URL` | `https://www.gstatic.com/generate_204` | 代理连通验证使用的固定外部目标 |
...
“系统设置”页面可以使用现有 AES-GCM 主密钥加密保存订阅地址。保存后系统会在私有 Mihomo volume 中生成受控运行配置，通过私有 Controller 热重载，等待订阅节点就绪并验证外部连通性。新配置失败时自动恢复上一份可用配置；清除页面订阅后重新加载 `MIHOMO_CONFIG_FILE`，默认即 DIRECT。
...
Provider 可以选择默认代理、直连或自定义代理。页面保存的订阅地址不会以明文返回。不要提交订阅地址或代理凭证。
...
[![AI Watch 系统架构](docs/diagrams/system-architecture.png)](docs/diagrams/system-architecture.png)

- 浏览器通过 HTTP API 和 SSE 使用 React/Vite 单页应用。
- AI Watch 容器合并宿主机只读配置、CC Switch 启动快照和 Redis 中的手填 Provider，再为每次任务生成隔离临时配置。
- Redis 保存设置、Provider 快照、加密配置、事件和运行元数据；Mihomo 提供仅 Compose 内部可达的代理出口与订阅热重载。
- 可靠性、SLO、事故、故障切换和通知都基于结构化且脱敏的请求事实，不读取通用 Redis Key 或原始 CLI 输出。

[可编辑的架构 draw.io 源文件](docs/diagrams/system-architecture.drawio)
...
截图命令固定使用 1440×960 视口、极昼主题和脱敏演示数据。draw.io 源文件位于 `docs/diagrams`，可使用 draw.io 桌面版重新导出同名 PNG。
